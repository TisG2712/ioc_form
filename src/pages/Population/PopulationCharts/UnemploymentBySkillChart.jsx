import React, { useState, useEffect, useCallback } from "react";
import {
  BarChart,
  Bar,
  XAxis,
  YAxis,
  Tooltip,
  Legend,
  ResponsiveContainer,
  Cell,
} from "recharts";
import { getTinhHinhLaoDongTheoQuy } from "../../../service/populationApi";
import { useDebouncedFilters } from "../../../utils/useDebounce";

// Hàm lấy tên tháng từ thangCapNhat
const getMonthName = (thangCapNhat) => {
  const monthNames = {
    "01": "Tháng 1", "02": "Tháng 2", "03": "Tháng 3", "04": "Tháng 4",
    "05": "Tháng 5", "06": "Tháng 6", "07": "Tháng 7", "08": "Tháng 8",
    "09": "Tháng 9", "10": "Tháng 10", "11": "Tháng 11", "12": "Tháng 12"
  };
  return monthNames[thangCapNhat] || `Tháng ${thangCapNhat}`;
};

function UnemploymentBySkillChart({ filters }) {
  // Sử dụng debounced filters để tránh request liên tục
  const debouncedFilters = useDebouncedFilters(filters, 100);
  
  // Sử dụng filters trực tiếp cho lần load đầu tiên
  // Sử dụng filters trực tiếp để đơn giản hóa
  const [chartData, setChartData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [isMobile, setIsMobile] = useState(false);

  // Hook để theo dõi kích thước màn hình
  const checkIsMobile = useCallback(() => {
    setIsMobile(window.innerWidth < 640);
  }, []);

  useEffect(() => {
    checkIsMobile();
    window.addEventListener('resize', checkIsMobile);
    
    return () => window.removeEventListener('resize', checkIsMobile);
  }, [checkIsMobile]);

  useEffect(() => {
    const fetchData = async () => {
      if (!filters || !filters.madvhc || filters.madvhc === null) {
        setChartData([]);
        return;
      }

      setLoading(true);
      setError(null);

      try {
        if (filters.quarter === "ALL") {
          // Khi chọn "Tất cả" ở dropdown quý, lấy dữ liệu cả năm theo từng quý
          const quarterlyData = [];
          
          for (let quarter = 1; quarter <= 4; quarter++) {
            try {
              const response = await getTinhHinhLaoDongTheoQuy({
                madvhc: filters.madvhc,
                quarter: quarter.toString(),
                year: filters.year
              });

              if (response.status === "OK" && response.body?.monthlyData) {
                // Tính trung bình tỷ lệ thất nghiệp theo trình độ của quý
                let totalUnemploymentRate = 0;
                let totalPopulation = 0;

                response.body.monthlyData.forEach((item) => {
                  totalPopulation += parseInt(item.nkTongNhanKhau);
                  // Tính tỷ lệ thất nghiệp trung bình từ các loại thất nghiệp
                  const unemploymentRate = (
                    parseFloat(item.thatNghiepTrenDHTyLe || 0) +
                    parseFloat(item.thatNghiepDHTyLe || 0) +
                    parseFloat(item.thatNghiepCDTyLe || 0) +
                    parseFloat(item.thatNghiepSCTyLe || 0) +
                    parseFloat(item.thatNghiepKhongChuyenMonTyLe || 0) +
                    parseFloat(item.thatNghiepChuaXacDinhTyLe || 0)
                  ) / 6;
                  totalUnemploymentRate += unemploymentRate;
                });

                const avgUnemploymentRate = Math.round((totalUnemploymentRate / response.body.monthlyData.length) * 10) / 10;

                quarterlyData.push({
                  name: `Q${quarter}`,
                  value: avgUnemploymentRate
                });
              } else {
                // Nếu không có dữ liệu, thêm quý với giá trị 0
                quarterlyData.push({
                  name: `Q${quarter}`,
                  value: 0
                });
              }
            } catch (err) {
              console.warn(`Error fetching unemployment data for Q${quarter}:`, err);
              // Nếu có lỗi, thêm quý với giá trị 0
              quarterlyData.push({
                name: `Q${quarter}`,
                value: 0
              });
            }
          }

          setChartData(quarterlyData);
        } else {
          // Khi chọn quý cụ thể, lấy dữ liệu theo tháng trong quý
          const response = await getTinhHinhLaoDongTheoQuy({
            madvhc: filters.madvhc,
            quarter: filters.quarter.replace('Q', ''),
            year: filters.year
          });

          if (response.status === "OK" && response.body?.monthlyData) {
            const processedData = response.body.monthlyData.map((item) => {
              const monthName = getMonthName(item.thangCapNhat);
              // Tính tỷ lệ thất nghiệp trung bình từ các loại thất nghiệp
              const unemploymentRate = (
                parseFloat(item.thatNghiepTrenDHTyLe || 0) +
                parseFloat(item.thatNghiepDHTyLe || 0) +
                parseFloat(item.thatNghiepCDTyLe || 0) +
                parseFloat(item.thatNghiepSCTyLe || 0) +
                parseFloat(item.thatNghiepKhongChuyenMonTyLe || 0) +
                parseFloat(item.thatNghiepChuaXacDinhTyLe || 0)
              ) / 6;
              
              return {
                month: monthName,
                value: Math.round(unemploymentRate * 10) / 10
              };
            });

            setChartData(processedData);
          } else {
            setError("Không có dữ liệu");
          }
        }
      } catch (err) {
        console.error("Error fetching unemployment data:", err);
        setError("Lỗi khi tải dữ liệu");
      } finally {
        setLoading(false);
      }
    };

    fetchData();
  }, [filters.madvhc, filters.quarter, filters.year, filters.timeId]);

  const COLORS = [
    "#3b82f6",
    "#10b981",
    "#f59e0b",
    "#f97316",
    "#ef4444",
    "#8b5cf6",
  ];

  if (loading) {
    return (
      <div className="flex items-center justify-center h-48">
        <div className="text-sm text-gray-500">Đang tải dữ liệu...</div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="flex items-center justify-center h-48">
        <div className="text-sm text-red-500">{error}</div>
      </div>
    );
  }

  if (!filters || filters.madvhc === "ALL") {
    return (
      <div className="flex items-center justify-center h-48">
        <div className="text-sm text-gray-500">Vui lòng chọn đơn vị hành chính để xem dữ liệu</div>
      </div>
    );
  }

  return (
    <div>
      <h2 className="text-xs font-medium mb-3 text-center">
        {filters.quarter === "ALL" 
          ? `Tỷ lệ dân cư thất nghiệp theo năng lực chuyên môn theo quý trong năm ${filters?.year}`
          : `Tỷ lệ dân cư thất nghiệp theo năng lực chuyên môn theo tháng trong quý ${filters?.quarter}`
        }
      </h2>
      <ResponsiveContainer width="100%" height={180}>
        <BarChart
          data={chartData}
          margin={{ top: isMobile ? 10 : 15, right: 10, left: -10, bottom: 5 }}
        >
          <XAxis 
            dataKey={filters.quarter === "ALL" ? "name" : "month"}
            tick={{ fontSize: 10 }} 
            angle={0}
            textAnchor="middle"
            height={30}
          />
          <YAxis 
            tick={{ fontSize: 10 }} 
            unit="%" 
            label={{ value: 'Tỷ lệ %', angle: 0, position: 'top', fontSize: 10 }}
          />
          <Tooltip
            contentStyle={{ fontSize: "10px" }}
            formatter={(value) => [`${value}%`, 'Tỷ lệ thất nghiệp']}
          />
          {/* <Legend wrapperStyle={{ fontSize: "10px" }} /> */}
          <Bar dataKey="value" name="Tỷ lệ thất nghiệp" fill="#ef4444" barSize={30} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
}

export default UnemploymentBySkillChart;
