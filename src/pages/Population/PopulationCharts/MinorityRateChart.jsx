...
function MinorityRateChart({ filters }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (filters.quarter === "ALL") {
          ...
          for (let quarter = 1; quarter <= 4; quarter++) {
            try {
              ...
              if (response.status === "OK" && response.body?.monthlyData) {
                // Tính trung bình tỷ lệ dân tộc thiểu số của quý
                let totalMinorityRate = 0;

                response.body.monthlyData.forEach((item) => {
                  const totalPopulation = parseInt(item.nkTongNhanKhau);
                  const minorityCount = parseInt(item.nkDanTocThieuSo || 0);
                  const minorityRate = totalPopulation > 0 ? (minorityCount / totalPopulation) * 100 : 0;
                  totalMinorityRate += minorityRate;
                });

                const avgMinorityRate = Math.round((totalMinorityRate / response.body.monthlyData.length) * 10) / 10;

                quarterlyData.push({
                  name: `Q${quarter}`,
                  "Tỷ lệ DTTS (%)": avgMinorityRate,
                });
              } else {
              ...
              console.warn(`Error fetching minority data for Q${quarter}:`, err);
          ...
          if (response.status === "OK" && response.body?.monthlyData) {
            const processedData = response.body.monthlyData.map((item) => {
              const totalPopulation = parseInt(item.nkTongNhanKhau);
              const minorityCount = parseInt(item.nkDanTocThieuSo || 0);
              const minorityRate = totalPopulation > 0 ? Math.round(((minorityCount / totalPopulation) * 100) * 10) / 10 : 0;
              
              return {
                name: getMonthName(item.thangCapNhat),
                "Tỷ lệ DTTS (%)": minorityRate,
              };
            });
        ...
        console.error("Error fetching minority rate data:", err);
  ...
  return (
    <div>
      <h2 className="text-xs font-medium mb-3 text-center">
        {filters.quarter === "ALL" 
          ? `Tỷ lệ đồng bào dân tộc thiểu số theo quý trong năm ${filters?.year}`
          : `Tỷ lệ đồng bào dân tộc thiểu số theo tháng trong quý ${filters?.quarter}`
        }
      </h2>
      <ResponsiveContainer width="100%" height={180}>
        <LineChart
          data={chartData}
          margin={{ top: isMobile ? 10 : 15, right: isMobile ? 5 : 10, left: isMobile ? 5 : -10, bottom: 7 }}
        >
          <XAxis 
            dataKey="name" 
          ...
          <Tooltip 
            ...
            formatter={(value) => [`${value}%`, 'Tỷ lệ DTTS']}
          />
...
export default MinorityRateChart;
