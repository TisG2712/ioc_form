...
function EducationStats4({ filters }) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (response && isMounted) {
          const chartData = [
            { name: "Giáo viên", value: response.tongGv || 0 },
            { name: "Học sinh", value: response.tongHs || 0 },
          ];
          setData(chartData);
        } else if (isMounted) {
        ...
        console.error("Error fetching data for EducationStats4:", error);
        if (isMounted) {
  ...
  const chartTitle = useMemo(() => {
    return `Tổng số giáo viên và học sinh (${filters?.maNamHoc || 'Năm học'})`;
  }, [filters?.maNamHoc]);
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-2 border border-gray-300 rounded shadow-sm text-[8px] sm:text-[10px]">
          <p className="text-gray-700">{`${payload[0].name}: ${payload[0].value.toLocaleString()} người`}</p>
        </div>
      );
    }
    return null;
...
export default EducationStats4;