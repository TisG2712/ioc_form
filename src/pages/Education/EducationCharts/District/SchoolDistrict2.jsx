...
function SchoolDistrict2({ filters }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (Array.isArray(response) && response.length > 0) {
          const filteredData = response
            ...
            .map(item => ({
              ...
              value: Math.round((item.soHocSinh || 0) / 1000) // Chia 1000 để dễ hiển thị
            }));
        ...
        console.error("Error fetching data for SchoolDistrict2:", error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <ResponsiveContainer width="100%" height={190}>
        ...
        >
          ...
          <Tooltip 
            ...
            formatter={(value) => [`${value}K`, 'Học sinh (nghìn)']} 
          />
          <Line type="monotone" dataKey="value" stroke="#f59e0b" strokeWidth={2} dot={{ fill: '#f59e0b', strokeWidth: 2, r: 4 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
...
export default SchoolDistrict2;
